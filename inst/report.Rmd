...
if(!is.na(spp_summ$pass)){kable(spp_summ$g.tab, "markdown", align="c", padding=2)}else{
  kable(data.frame(Error = "Model did not pass posterior predictive checks"), format = "markdown", align="c")
}
```
...
if(!is.na(spp_summ$pass)){BayesCorrOcc::MapDiff(alpha = spp_summ$spp_alpha)}
```
...
if(!is.na(spp_summ$pass)){BayesCorrOcc::PlotLat(alpha = spp_summ$spp_alpha)}
```
...
if(!is.na(spp_summ$pass)){BayesCorrOcc::PlotLon(alpha = spp_summ$spp_alpha)}
```
